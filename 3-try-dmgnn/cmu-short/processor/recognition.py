...
class REC_Processor(Processor):
    ...
    def train(self):
        ...
        encoder_inputs, decoder_inputs, targets = train_sample(normed_train_dict, 
                                                               ...
                                                               len(self.dim_use)) # len = 40, each data's size is (xxx,78)
        # encoder_inputs size: (32,49,78) , decoder_inputs size (32,1,78), targets size (32,10,78)

        
        encoder_inputs_v = np.zeros_like(encoder_inputs) # size : (32,49,78)
        ...
        #decoder_inputs_previous = torch.Tensor(encoder_inputs).unsqueeze(1).to(self.dev)
        #decoder_inputs_previous2 = torch.Tensor(encoder_inputs).unsqueeze(1).to(self.dev)
    ...
    def test(self, evaluation=True, iter_time=0, save_motion=False, phase=False):
        ...
        for action_num, action in enumerate(self.actions):
            ...
            decoder_inputs_previous2 = torch.Tensor(encoder_inputs[:, -2, :]).unsqueeze(1).to(self.dev)
            #decoder_inputs_previous = torch.Tensor(encoder_inputs).unsqueeze(1).to(self.dev)   # [N,T,D] = [64,  1, 63]
            #decoder_inputs_previous2 = torch.Tensor(encoder_inputs).unsqueeze(1).to(self.dev)
