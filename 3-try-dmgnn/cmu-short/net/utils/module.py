...
class AveargeJoint(nn.Module):
    ...
    def __init__(self):
        ...
        '''
    def forward(self, x):
        x_part = torch.tensor([]).cuda()
        
        for i in range(0,40):
            x_temp1 = F.avg_pool2d(x[:, :, :, (i*5,i*5+1,i*5+2,i*5+3,i*5+4)], kernel_size=(1, 5))
...
class AveargePart(nn.Module):
    ...
    def forward(self, x):
        x_body = torch.tensor([]).cuda()
        for i in range(0,20):
            x_temp2 = F.avg_pool2d(x[:, :, :, (i*10,i*10+1,i*10+2,i*10+3,i*10+4,i*10+5,i*10+6,i*10+7,i*10+8,i*10+9)], kernel_size=(1, 10))
            x_body = torch.cat((x_body,x_temp2),dim = -1)
